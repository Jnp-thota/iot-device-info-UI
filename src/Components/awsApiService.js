...
export const summaryApi = {
  getSummary: async () => {
    return makeApiRequest('devices/summary');
  },
};
...
export const devicesApi = {
  ...
  create: (data) => {
    const { id, ...requestData } = data; // Remove id from the request body if it exists
    return makeApiRequest('devices', 'POST', requestData);
  },

  update: (id, data) => {
    const { id: _, ...requestData } = data; // Remove id from the request body if it exists
